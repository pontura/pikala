...
public class SlingShot : MonoBehaviour
{
    ...
    [HideInInspector]
    //the bird to throw
    public GameObject BananaToThrow;

    //the position of the bird tied to the slingshot
    public Transform BananaWaitPosition;
    ...
    void Start()
    {
        ...
        SlingshotMiddleVector = new Vector3((LeftSlingshotOrigin.position.x + RightSlingshotOrigin.position.x) / 2,
            (LeftSlingshotOrigin.position.y + RightSlingshotOrigin.position.y) / 2, -1);
    }
    ...
    private Vector3 lastPushPosition;
        ...
        switch (slingshotState)
        {
            case SlingshotState.Idle:
                ...
                if (Input.GetMouseButtonDown(0))
                {
                    ...
                    if (BananaToThrow.GetComponent<CircleCollider2D>() == Physics2D.OverlapPoint(location))
                    {
            ...
            case SlingshotState.UserPulling:
                ...
                {
                    Events.MonkeysAiming(true);
                    location = Camera.main.ScreenToWorldPoint(Input.mousePosition);

                    location.z = 0;
                    CheckMove(Vector3.Distance(location, lastPushPosition));
                    lastPushPosition = location;
                    ...
                    if (Vector3.Distance(location, SlingshotMiddleVector) > 1.5f)
                    {
                        ...
                        maxPosition.z = 0;
                        BananaToThrow.transform.position = maxPosition;
                    }
                    else
                    {
                        BananaToThrow.transform.position = location;
                    }
                    float distance = Vector3.Distance(SlingshotMiddleVector, BananaToThrow.transform.position);
                ...
                else//user has removed the tap 
                {
                    ...
                    //{
                        ...
                        slingshotState = SlingshotState.BananaFlying;
                        ThrowBanana(distance);
                   // }
                    ...
                    //    BananaToThrow.transform.positionTo(distance / 10, //duration
                    //        BananaWaitPosition.transform.position). //final position
                ...
                break;
            case SlingshotState.BananaFlying:
                break;
            default:
                break;
        }
    ...
    float lastMove;
    void CheckMove(float newMove)
    {
        if(newMove == lastMove) return;
        //lastMove = newMove;
        //if (newMove == 0)
        //    Invoke("StopDelay", 1f);
        //else
    ...
    void StopDelay()
    {
        if (lastPushPosition == location) return;
        Events.MonkeysAiming(false);
    }
    private void ThrowBanana(float distance)
    {
        Events.OnSoundFX("throw");
        //get velocity
        Vector3 velocity = SlingshotMiddleVector - BananaToThrow.transform.position;
        BananaToThrow.GetComponent<Banana>().OnThrow(); //make the bird aware of it
        //old and alternative way
        //BananaToThrow.GetComponent<Rigidbody2D>().AddForce
        //    (new Vector2(v2.x, v2.y) * ThrowSpeed * distance * 300 * Time.deltaTime);
        //set the velocity
        BananaToThrow.GetComponent<Rigidbody2D>().velocity = new Vector2(velocity.x, velocity.y) * ThrowSpeed * distance;


        //notify interested parties that the bird was thrown
        if (BananaThrown != null)
            BananaThrown(this, EventArgs.Empty);
    }

    public event EventHandler BananaThrown;

    private void InitializeBanana()
    {
        //initialization of the ready to be thrown bird
        BananaToThrow.transform.position = BananaWaitPosition.position;
        slingshotState = SlingshotState.Idle;
    ...
    {
        SlingshotLineRenderer1.SetPosition(1, BananaToThrow.transform.position);
        SlingshotLineRenderer2.SetPosition(1, BananaToThrow.transform.position);
    }
    ...
    void DisplayTrajectoryLineRenderer2(float distance)
    {
        ...
        Vector3 v2 = SlingshotMiddleVector - BananaToThrow.transform.position;
        int segmentCount = 7;
        float segmentScale = 2;
        Vector2[] segments = new Vector2[segmentCount];

        // The first line point is wherever the player's cannon, etc is
        segments[0] = BananaToThrow.transform.position;
